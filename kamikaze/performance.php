...
<p>
The architecture of Kamikaze version 3.0.0 is based on the version 1.0.0 and version 2.0.0. The version 1.0.0 and version 2.0.0 provide an excellent platform to do set operatons on document lists. Version 3.0.0 focus on optimizing PForDelta compression algorithm and the correponding set operations. In this page, we provide the experimental results as follows:

<ul>
<li>We compare the relative performance of Kamikaze the version 2.0.0 and versoin 3.0.0. </li>
<li>We provide the absolute performance evaluation of Kamikaze versoin 3.0.0. </li>
</ul>
...
is important for quickly iterating all docIds in the inverted lists. The random lookup speed is crucial to find certain docIds in the inverted list. Since the doc sets are built on top of the PForDelta-based index, the decompression performance of PForDelta plays an important role in the overall set operation speed. In contrast, compression speed is less critical since each inverted list is compressed only once during index building, and then 
...
<li>We call versoin 3.0.0 the NEW version and version 2.0.0 the OLD version. 
...
<p>In Figure 1.3, we show the compression results in the number of bits (on average) per docId, which is equal to the serialized size divided by the list length. From Figure 1.3, we can see the following:
<ul>
<li> The relative performane of the OLD and NEW versions are the same as in Figure 1.2.
<li> Since the docIds in the three lists are distributed very sparsely (recall the range is [0,75M)), the d-gaps(differences of consecutive docIds) are on average very large values. As a result, we have to use more bits to represent them (about 15 bits for the NEW version and over 30 bits for the OLD version). Please note that the numbers shown in Figure 1.3 is the serialized object size per docId instead of just the PForDelta-related Bits/Id (It contains other auxiliary data than docIds and thus is larger than the numbers reported in other literatures).   
</ul> 
</p>
<p align="center">
...
<p>
In Figure 2.5, we show the corresponding iteration speed in the number of processed docIds per second for both OLD and NEW versions. We can see that the NEW version can iterate about 75-90 million docIds per second on the compressed lists.
</p>
<p align="center">
  <img src = "images/Exp2-speed.png" width="800px" />
</p>

<b> (3) Experiment 3 </b><p>We run similar experiments on three even longer lists, each of which is composed of 1,500,000, 3,750,000 and 7,500,000 numbers out of 75M positive integers. The experimental results are shown in Figure 3.1 through Figure 3.5. From these Figures, we can see that the NEW version can achieve about 20%-30% improvement of various performances over the OLD version. The improvement percentage is slightly decreased since the docIds on longer lists are much more densely distributed, resulting in a large amount of very small d-gaps which cannot be compressed too much anyway. 
We note that in Figure 3.3, even less number of bits (than those in Figure 2.3 and Figure 1.3) are needed to encode a docId since the data are more densely distributed. 
